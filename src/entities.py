...
class Sprite:
    def __init__(self, path, num_frames, anim_speed):
        self.images = imgload(path, S, num_frames)
        self.anim = 0
...
create_entity(
    Transform([-100, -400], [2, 0.0], glob.gravity),
...
class RenderSystem:
    ...
    def process(self, scroll, world):
        for tr, sprite in self.get_components():
            ...
            extended_rect = inflate_keep(sprite.rect, 20)
            ...
            try:
                ...
                self.display.blit(sprite.images[int(sprite.anim)], blit_pos)